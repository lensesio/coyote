...
const html = `<html lang="en">
	<head>
	    ...
	    <tr>
	      ...
	      <td class="tg-zczf">{{ $v.Command }}</td>
	      <td class="tg-zczf">{{ if showmore $v.Stdout }}
                <div class="content hideContent">{{ range $v.Stdout }}{{ . }}</br>{{ end }}</div><div class="show-more"><a href="#">Show more</a></div>{{ else }}
                {{ range $v.Stdout }}{{ . }}</br>{{ end }}{{ end }}
              </td>
	      <td class="tg-zczf">{{ if showmore $v.Stderr }}
                <div class="content hideContent">{{ range $v.Stderr }}{{ . }}</br>{{ end }}</div><div class="show-more"><a href="#">Show more</a></div>{{ else }}
                {{ range $v.Stderr }}{{ . }}</br>{{ end }}{{ end }}
              </td>
              {{ else }}
